...
def test_and_plot_distribution_multi_model(model_list, model_name_list, is_transformer_list, datasets, vocab_dict_list, prediction_len, batch_size=32, iteration=""):
    ...
    # with open(str(ITER)+'iter-model-feature-num-comparison-results.p', 'wb') as f_results:
    #     pickle.dump(model_results, f_results, pickle.HIGHEST_PROTOCOL)
    
    with open(str(ITER)+'iter-model-feature-num-comparison-results.p', 'rb') as f_results:
    ...
    with open('NEWDatasets/combined-dataset-preprocessed/6col-VocabBackDict.p', 'rb') as f_vocab_back:
        regular_vocab_back_dict = pickle.load(f_vocab_back)
    with open('../Transformer-training-fewer-features/NEWDatasets/combined-dataset-preprocessed-3col/3col-VocabBackDict.p', 'rb') as f_vocab_back:
        few_feature_vocab_back_dict = pickle.load(f_vocab_back)
    vocab_back_list = [few_feature_vocab_back_dict, regular_vocab_back_dict]
    ...
    plt.savefig(f'feature_num_bucket_distance_comparison_PDF.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout(pad=1.0)
    plt.savefig(f'feature_num_per_column_distances_comparison.png')
    plt.show()
...
with open('NEWDatasets/combined-dataset-preprocessed/6col-rtt-based-test.p', 'rb') as f:
    regular_feature_dataset = pickle.load(f)
with open('../Transformer-training-fewer-features/NEWDatasets/combined-dataset-preprocessed-3col/3col-rtt-based-test.p', 'rb') as f:
    few_feature_dataset = pickle.load(f)
prediction_len = 10
test_and_plot_distribution_multi_model(model_list, model_name_list, is_transformer_list, [few_feature_dataset, regular_feature_dataset], [few_feature_vocab_dict, regular_vocab_dict], prediction_len, batch_size=32, iteration=ITER)
