...
with open('./NEWDatasets/'+dataset_name+'.p', 'rb') as f:
    ...
    new_train_dataset = torch.zeros_like(train_dataset)

    # Replace each value in the 6th column with the sum of the last 10 values
    for i in range(train_dataset.shape[0]):
        for j in range(train_dataset.shape[1]):
            if j >= 10:
                new_train_dataset[i, j, column_index] = torch.sum(train_dataset[i, j-10:j, column_index])
            else:
                new_train_dataset[i, j, column_index] = torch.sum(train_dataset[i, :j, column_index])
