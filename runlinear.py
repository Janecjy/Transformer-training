...
with open('./NEWDatasets/FullDataset-train.p', 'rb') as f:
    train_dataset = pickle.load(f)
model = DLinear(seq_len=CONTEXT_LENGTH, pred_len=PREDICTION_LENGTH, individual=True).to(DEVICE)
train_dataset = train_dataset.to(DEVICE)

opt = torch.optim.Adam(model.parameters(), lr=1e-4)
trained_model, loss_traj = train_linear_model(model, train_dataset, opt, PREDICTION_LENGTH, DEVICE, 1000, BATCH_SIZE)

torch.save(trained_model, './Models/DLinear-individual-1000iter.p')